...
# if len(sys.argv)<2:
# 	print "usage:"
# 	print "BB_PslMappedLength.py in.bed other_in_1.bed other_in_2.bed ......"
# 	print "\033[1;31;38m"
# 	print "="*50
# 	print "\033[0m"
# 	os._exit(0)
...
##run##
d_file={}
l_sp=["mm10","bosTau8","calJac3","galGal4","hg38","monDom5","ornAna1","oryCun2","rheMac3","rn6","susScr3"]
for i in l_sp:
	d_file[i]=open("/data/tongji2/piRNA/Output/PslMap/mm10_%s.bed"%i,"r")

d_len={}
while True:
	line=d_file["mm10"].readline().split()
	if not line:
		break
	d_len[line[3]]=float(line[4])

for i in l_sp:
	d_out=open("/data/tongji2/piRNA/Output/PslMap/temp_mm10_%s.bed"%i,"w")
	while True:
		line=d_file[i].readline().split()
		if not line:
			break
		d_out.write("%s\t%s\t%s\t%s\t%s\t%s\n"%(line[0],line[1],line[2],line[3],round(float(line[4])/d_len[line[3]],2),line[5]))
	d_out.close()
