...
f_name=open(sys.argv[3],"r")
f_mature=open(sys.argv[4],"r")
d_mature={}
while True:
	l=f_mature.readline().split()
	if not l:
		break
	d_mature[l[0]]=[]
	for i in range(1,len(l)):
		d_mature[l[0]].append(l[i])
l_name=[]
while True:
...
while True:
	...
	if not l:
		break
	p=re.findall(r'cluster\:\w+\:([\w\.\-]+)\:',l[3])
	if len(p)>0 and int(l[2])-int(l[1])>40:
		if (l[5]=="+" and l[1] in d_mature[p[0]]) or (l[5]=="-" and l[2] in d_mature[p[0]]):
			d[p[0]]+=(1/float(l[4]))
