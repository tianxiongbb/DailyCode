...
if len(sys.argv)<2:
	print "usage:"
	print "BB_IntersectBed2Bed.py in.bed2 in.bed out.prefix plus/minus feature.list"
	print "\033[1;31;38m"
	print "="*50
	print "\033[0m"
	os._exit(0)
...
##run##
if sys.argv[4]=="minus":
	command="bedtools intersect -S -f 0.5 -wo -a %s -b %s > %s.intersect"%(sys.argv[1],sys.argv[2],sys.argv[3])
else:
	command="bedtools intersect -s -f 0.5 -wo -a %s -b %s > %s.intersect"%(sys.argv[1],sys.argv[2],sys.argv[3])

os.system(command)

command="BB_IntersectSignal_Apr21.py %s.intersect %s.signal %s"%(sys.argv[3],sys.argv[3],sys.argv[5])
os.system(command)
