...
help_info(){
	...
	echo "    -p pvalue for peak calling, if set, qvalue will not implement. --default: not set"
...
# read parameters
while getopts "hvt:c:o:C:q:p:f:g:" OPTION; do
	case $OPTION in
		h)  help_info;;
...
if [ -n "${TREAT_FILE2}" ];then
	...
	set -x
	macs2 callpeak -t ${TREAT_FILE1} ${TREAT_FILE2} -c ${CONTROL_FILE1} ${CONTROL_FILE2} -f ${FILE_FORMAT} -g ${GENOME} --keep-dup 1 --outdir ${OUT_DIR} -n ${OUT_NAME}_rep0 -B > ${LOG_DIR}/macs2_callpeak_rep0.log 2>&1
	set +x
	echo0 1 "run macs2 callpeak for rep1\t"`date`
	set -x
	macs2 callpeak -t ${TREAT_FILE1} -c ${CONTROL_FILE1} -f ${FILE_FORMAT} -g ${GENOME} --keep-dup 1 --outdir ${OUT_DIR} -n ${OUT_NAME}_rep1 -B > ${LOG_DIR}/macs2_callpeak_rep1.log 2>&1
	set +x
	echo0 1 "run macs2 callpeak for rep2\t"`date`
	set -x
	macs2 callpeak -t ${TREAT_FILE2} -c ${CONTROL_FILE2} -f ${FILE_FORMAT} -g ${GENOME} --keep-dup 1 --outdir ${OUT_DIR} -n ${OUT_NAME}_rep2 -B > ${LOG_DIR}/macs2_callpeak_rep2.log 2>&1
	set +x
...
EOF
	...
	echo0 1 "run macs2 bdgcmp to get FE and pvalue signal\t"`date`
	set -x
	macs2 bdgcmp -t ${OUT_PREFIX}_rep1_treat_pileup.bdg -c ${OUT_PREFIX}_rep1_control_lambda.bdg -m FE ppois --outdir ${OUT_DIR} -o ${OUT_NAME}_rep1.FE.bdg ${OUT_NAME}_rep1.pvalue.bdg > ${LOG_DIR}/macs2_bdgcmp_rep1.log 2>&1
	set +x
	echo0 1 "make bigWig files for visulization\t"`date`
	sort -k1,1 -k2,2n ${OUT_PREFIX}_rep1.FE.bdg > ${OUT_PREFIX}_rep1.FE.sort.bdg
	bedGraphToBigWig ${OUT_PREFIX}_rep1.FE.sort.bdg ${CHROM_SIZE} ${OUT_PREFIX}_rep1.FE.bw
	sort -k1,1 -k2,2n ${OUT_PREFIX}_rep1.pvalue.bdg > ${OUT_PREFIX}_rep1.pvalue.sort.bdg
	bedGraphToBigWig ${OUT_PREFIX}_rep1.pvalue.sort.bdg ${CHROM_SIZE} ${OUT_PREFIX}_rep1.pvalue.bw
	# summary
	cat << EOF > ${OUT_DIR}/plot.R
#!/usr/bin/env Rscript
###library packages###
