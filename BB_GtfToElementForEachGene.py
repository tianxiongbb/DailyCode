...
if len(sys.argv)<2:
	print "usage:"
	print sys.argv[0]+" in.gtf out.bed exon/gene/transcript/five_prime_utr"
	print "\033[1;31;38m"
	print "="*50
	print "\033[0m"
	os._exit(0)
...
while True:
	...
	gt=re.findall(r'gene_biotype \"([\w\.\-\_]+)\"',li)[0]
	if len(gn)==0:
		gn=gi
	genename=gi
	if l[2]==sys.argv[3] and len(genename)>0:
                if genename[0] not in dict_exons:
                        dict_exons[genename[0]]=[]
                dict_exons[genename[0]].append([l[0],int(l[3]),int(l[4]),l[6],gt,gn[0]])
