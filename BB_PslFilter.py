...
if len(sys.argv)<2:
	print "usage:"
	print "BB_PslFilter.py in.psl out.psl p1(percent mapped,eg 0.95) p2(percent mismatch, eg 0.01)"
	print ""
	print "The program will filter psl records"
	print "which are >p1 mapped and <p2 mismatch"
	print "\033[1;31;38m"
	print "="*50
	print "\033[0m"
	os._exit(0)
...
file_psl_before_filter=open(sys.argv[1],"r")

for i in range(5):
	file_psl_before_filter.readline()

list_psl_before_filter=read_file_into_list(file_psl_before_filter)
list_psl_after_filter=[]
p1=0.95
p2=0.01
...
print "Read %s records in~"%len(list_psl_before_filter)
for i in list_psl_before_filter:
	if float(i[0])/float(i[10])>p1 and float(i[1])/float(i[0])<p2:
		list_psl_after_filter.append(i)

print "Write %s records out~"%len(list_psl_after_filter)

file_psl_after_filter=open(sys.argv[2],"w")
write_list_into_file(list_psl_after_filter,file_psl_after_filter)
