...
if len(sys.argv)<2:
	print "usage:"
	print "BB_Bed2Length.py in.bed2 out.length length_range(eg:24-32)"
	print "\033[1;31;38m"
	print "="*50
	print "\033[0m"
	os._exit(0)
...
s=int(sys.argv[3].split("-")[0])
e=int(sys.argv[3].split("-")[1])+1

l_in=read_file_into_list(f_in)
d_out={}
...
for i in l_in:
	if not d_out.has_key(len(i[6])):
		continue
	d_out[len(i[6])]+=float(i[3])/float(i[4])
