...
if len(sys.argv)<2:
	print "usage:"
	print "python BB_GtfToBed6.py in.gtf out.bed id/name"
	print "\033[1;31;38m"
	print "="*50
	print "\033[0m"
	os._exit(0)
...
##run##
f_gtf=open(sys.argv[1],"r")
f_bed=open(sys.argv[2],"w")

while True:
	l=f_gtf.readline().split("\t")
	if not l[0]:
		break
	if l[0][0]=="#":
		continue
	gene_Id=re.findall(r'gene_id \"([\w\.\-\_]+)\"',l[8])
	gene_name=re.findall(r'gene_name \"([\w\.\-\_]+)\"',l[8])
	trans_id=re.findall(r'transcript_id \"([\w\.\-\_]+)\"',l[8])
	if len(trans_id)==0:
		trans_id=["NONE"]
	if len(gene_name)==0:
		gene_name=gene_Id
	...
	tn=trans_id[0]
	if l[2]=="exon":
		f_bed.write("%s\t%s\t%s\t%s\t%s\t%s\n"%(l[0],l[3],l[4],gn,tn,l[6]))
